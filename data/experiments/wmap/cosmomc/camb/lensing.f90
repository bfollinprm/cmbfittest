...
!flat method stores
 ...
 integer, parameter :: lensed_convolution_margin = 0
...
!    do while(lSamp%l(max_lensed_ix) > CP%Max_l -250)
    ...
    !print*,'lmin=',lmin,'lmax_lensed=',lmax_lensed
    !print*, max_lensed_ix
    !print*, lSamp%l(0:max_lensed_ix)
    !print*, CP%Max_l
    !pause
    ...
    do l=lmin,CP%Max_l
     ! (2*l+1)l(l+1)/4pi C_phi_phi: Cl_scalar(l,1,C_Phi) is l^4 C_phi_phi
       Cphil3(l) = CP%alens * Cl_scalar(l,in,C_Phi)*(2*l+1)*(l+1)/real(l,dl)**3/(4*pi) 
    ...
    if (lmax > CP%Max_l) then
     ...
     do l=CP%Max_l+1, lmax
       ...
       sc = (2*l+1)/(4*pi) * 2*pi/(l*(l+1))  
       Cphil3(l) = CP%alens * highL_CL_template(l, C_Phi)*fac*sc
