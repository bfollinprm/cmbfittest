...
!if CMBdataset%has_xfactors then obs, var and N_inv are for the offset-lognormal variables Z
  ...
  Type CMBdataset
    ...
    logical :: has_tSZ_template
    logical :: has_kSZ_template
    logical :: has_ps_template
    logical :: has_cl_template
    ...
    real, pointer, dimension(:) :: tsz_template
    real, pointer, dimension(:) :: ksz_template
    real, pointer, dimension(:) :: ps_template
    real, pointer, dimension(:) :: cl_template
...
100    stop 'Error reading all_l_file file'
 ...
 subroutine ReadDataset(aname)
   ...
   aset%has_tsz_template = .false.
   aset%has_ksz_template = .false.
   aset%has_ps_template = .false.
   aset%has_cl_template = .false.
...
!Special cases
 ...
 subroutine ReadtSZTemplate(aset, aname, ascale)
    Type (CMBdataset) :: aset
    ...
    real sz, n3000
     ...
     allocate(aset%tsz_template(2:lmax))
     aset%tsz_template = 0
     aset%has_tsz_template = .true.
     call OpenTxtFile(aname, tmp_file_unit)
     do
       read(tmp_file_unit,*,end=2) l, sz
       if (l>=2 .and. l<=lmax) aset%tsz_template(l) = sz
     end do
     
2    Close(tmp_file_unit)

     if (lmax >= 3000) then
        n3000 = ascale/aset%tsz_template(3000)
     else 
        write(*,*)'Not normalized to D3000=1'
        n3000 = ascale
     end if
     do l=2,lmax 
        aset%tsz_template(l) = n3000 * aset%tsz_template(l) /(l*(l+1)/twopi)
     end do
 end subroutine ReadtSZTemplate

 subroutine ReadkSZTemplate(aset, aname, ascale)
    Type (CMBdataset) :: aset
    real, intent(in) :: ascale
    character(LEN=*), intent(IN) :: aname
    integer l, unit
    real sz, n3000
     ...
     allocate(aset%ksz_template(2:lmax))
     aset%ksz_template = 0
     aset%has_ksz_template = .true.
     call OpenTxtFile(aname, tmp_file_unit)
     do
       read(tmp_file_unit,*,end=2) l, sz
       if (l>=2 .and. l<=lmax) aset%ksz_template(l) = sz
     end do
     
2    Close(tmp_file_unit)

     if (lmax >= 3000) then
        n3000 = ascale/aset%ksz_template(3000)
     else 
        write(*,*)'Not normalized to D3000=1'
        n3000 = ascale
     end if
     do l=2,lmax 
        aset%ksz_template(l) = n3000 * aset%ksz_template(l) /(l*(l+1)/twopi)
     end do
 end subroutine ReadkSZTemplate

 subroutine SetPSTemplate(aset, ascale)
   Type (CMBdataset) :: aset
   real, intent(in) :: ascale
   integer :: l
   REAL :: n3000
   
   allocate(aset%ps_template(2:lmax))
   aset%ps_template = 0
   aset%has_ps_template = .true.
  
   n3000 = twopi / (3000.*(3001.)) * ascale
   do l=2,lmax 
      aset%ps_template(l) = n3000 
   end do
 end subroutine SetPSTemplate

  subroutine ReadCLTemplate(aset, aname, ascale)
    Type (CMBdataset) :: aset
    real, intent(in) :: ascale
    character(LEN=*), intent(IN) :: aname
    integer l, unit
    real clus, n3000
     allocate(aset%cl_template(2:lmax))
     aset%cl_template = 0
     aset%has_cl_template = .true.
     call OpenTxtFile(aname, tmp_file_unit)
     do
       read(tmp_file_unit,*,end=4) l, clus
       if (l>=2 .and. l<=lmax) aset%cl_template(l) = clus
     end do
     
4    Close(tmp_file_unit)
     
     if (lmax >= 3000) then
        n3000 = ascale/aset%cl_template(3000)
     else 
        write(*,*)'Not normalized to D3000=1'
        n3000 = ascale
     end if
     do l=2,lmax 
        aset%cl_template(l) = n3000 * aset%cl_template(l) /(l*(l+1)/twopi)
     end do
 end subroutine ReadCLTemplate
...
101 call CloseFile(file_unit)
 ...
 function CMBLnLike(cl, freq_params, nuisance_params)
  ...
  real tsznorm, ksznorm, psnorm, clnorm, szcl(lmax,num_cls_tot)
  integer i,l
  integer nuisance
  ...
  tsznorm = freq_params(1)
  ksznorm = freq_params(2)
  psnorm = freq_params(3)  
  clnorm = freq_params(4)

  nuisance =1
  do i=1, num_datasets
     szcl= cl
     if (datasets(i)%has_tsz_template) then
      szcl(2:lmax,1) = szcl(2:lmax,1) + tsznorm*datasets(i)%tsz_template(2:lmax)
     end if
     if (datasets(i)%has_ksz_template) then
      szcl(2:lmax,1) = szcl(2:lmax,1) + ksznorm*datasets(i)%ksz_template(2:lmax)  
     end if
     if (datasets(i)%has_ps_template) then
      szcl(2:lmax,1) = szcl(2:lmax,1) + psnorm*datasets(i)%ps_template(2:lmax)  
     end if
     if (datasets(i)%has_cl_template) then
      szcl(2:lmax,1) = szcl(2:lmax,1) + clnorm*datasets(i)%cl_template(2:lmax)
     end if
#ifdef _SAVE_BANDPOWERS_
     !warning - not sure why but this seems to infinite loop
     !or introduces one elsewhere
     if (i .eq. 1) then
        open (33,file='suxp_cls')
        do l=2,lmax
           write(33,*)l,l*(l+1)*szcl(l,1)/6.28318530718
        enddo
        close(33)
...
!  print*, 'MASTER TTTT             ', 2*like(ttlike),     ttmax-lowl_max
!  print*, 'Beam/ptsrc TT correction', 2*like(beamlike)
!  if ( use_gibbs ) then
!     print*, 'low-l TTTT gibbs        ', 2*like(ttlowllike)
!  else
!     print*, 'low-l TTTT chi2         ', 2*like(ttlowllike), tt_npix
!     print*, 'low-l TTTT det          ', 2*like(ttlowldet)
!  end if
!  print*, 'MASTER TETE chi2        ', 2*like(telike),     temax-23
!  print*, 'MASTER TETE det         ', 2*like(tedet)
!  print*, 'TT/TE/EE/BB low-l chi2  ', 2*like(lowllike),   teeebb_npix
 ! print*, 'TT/TE/EE/BB low-l det   ', 2*like(lowldet)
 ! print*, 'TOTAL -2ln(L)           ', 2*sum(like)
 ! print*,'all :',-2*like
