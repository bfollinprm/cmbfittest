...
!Make these multiples of 50, should be 50 more than you need accurately
  integer, parameter :: lmax = 3300, lmax_tensor = 400
...
!Only used in params_CMB
   ...
   integer :: bbn_type, bbn_fitting
   character(100) :: chain_prefix
...
!if num_cls=4 and CMB_lensing then increased to 4 
  ...
  !! adding SNLS parameters for marginalization afterwards - spt.lps12
  INTEGER, PARAMETER :: norm_alpha=norm_freq_ix+num_freq_params
  ...
  Type CMBParams
     ...
     real w
     real YHe, nnu
     real ombh2_bbn, nnu_bbn
     real logDoH
     real aeisw, alens
     real sum_mnu
     real z_star, z_eq, thetaS, thetaD, DoS
     real reserved(5)
