...
! This is a driving routine that illustrates the use of the program.
        ...
        use IO
        use ParamNames
        use SNLS, ONLY : read_snls_dataset, snls_prep, snls_cleanup  !! load SNLS module - spt.lps12
!        use fpico
#ifdef WMAP_PARAMS
        use WMAP_OPTIONS
#endif
        ...
        character(LEN=Ini_max_string_len) baseroot, filename(100), &
         mpk_filename(100), tSZTemplate(100), kSZTemplate(100), PSTemplate(100), CLTemplate(100), &
         numstr, fname, keyname
        integer numbaosets
        character(LEN=Ini_max_string_len) bao_filename(100)

        CHARACTER(LEN=Ini_max_string_len) :: snls_filename !! Add a variable snls_filename - spt.lps12

        real tSZscale(100), kSZscale(100), PSscale(100), CLscale(100)
       ...
       instance = 0 
        ...
        !chain_prefix = ReadIniFileName(DefIni,'chain_prefix')
        !chain_prefix = adjustl(chain_prefix)
...
!          call CreateOpenFile(fname,indepfile_unit,'unformatted',.not. new_chains)
        ...
        Use_SNLS = Ini_Read_Logical('use_SNLS',.false.) !! Add check for Use_SNLS from Ini file - spt.lps12
        ...
        bbn_type = Ini_Read_Int('BBN_type')
        bbn_fitting = Ini_Read_Int('BBN_fitting',2)
        use_Yp_prior = Ini_Read_Logical('use_Yp_prior',.false.)
        if (use_Yp_prior) then
            write(*,*) "using Yp prior"
            Yp_mean = Ini_read_Real('Yp_mean')
            Yp_stdev = Ini_read_Real('Yp_stdev')
        endif
        use_DoH_prior = Ini_Read_Logical('use_DoH_prior',.false.)
        if (use_DoH_prior) then
            write(*,*) "using D/H prior"
            DoH_mean = Ini_read_Real('DoH_mean')
            DoH_stdev = Ini_read_Real('DoH_stdev')
        endif
        use_DoS_prior = Ini_Read_Logical('use_DoS_prior',.false.)
        if (use_DoS_prior) then
            write(*,*) "using theta_D/theta_S prior"
            DoS_mean = Ini_read_Real('DoS_mean')
            DoS_stdev = Ini_read_Real('DoS_stdev')
        endif
        
        use_H0_prior = Ini_Read_Logical('use_H0_prior',.false.)
        if (use_H0_prior) then
            write(*,*) "using H0 prior"
            H0_mean = Ini_read_Real('H0_mean')
            H0_stdev = Ini_read_Real('H0_stdev')
        endif

        use_tau_prior = Ini_Read_Logical('use_tau_prior',.false.)
        if (use_tau_prior) then
            write(*,*) "using tau prior"
            tau_mean = Ini_read_Real('tau_mean')
            tau_stdev = Ini_read_Real('tau_stdev')
        endif
        ...
        if (Use_CMB) then
         do i= 1, numsets
          ...
          keyname=numcat('cmb_dataset_tSZ',i)
          tSZTemplate(i) = ''
          if (Ini_HasKey(KeyName)) tSZTemplate(i) = Ini_Read_String(keyname, .false.) 
          if (tSZTemplate(i)/='') then
           tSZScale(i) = Ini_read_Real(numcat('cmb_dataset_tSZ_scale',i),1.0)
           call ReadtSZTemplate(datasets(i), tSZTemplate(i), tSZScale(i))
           use_SZ_prior = Ini_Read_Logical('use_SZ_prior',.false.)
           sz_mean = Ini_read_Real(numcat('SZ_mean',i),sz_mean_default)
           sz_stdev = Ini_read_Real(numcat('SZ_stdev',i),sz_stdev_default)
          end if

          kSZTemplate(i) = Ini_Read_String(numcat('cmb_dataset_kSZ',i), .false.) 
          if (kSZTemplate(i)/='') then
           kSZScale(i) = Ini_read_Real(numcat('cmb_dataset_kSZ_scale',i),1.0)
           call ReadkSZTemplate(datasets(i), kSZTemplate(i), kSZScale(i))
          end if
          
          PSScale(i) = Ini_read_Real(numcat('cmb_dataset_PS_scale',i),0.0)
          if (PSScale(i) > 0) then
             call SetPSTemplate(datasets(i), PSScale(i))
             use_PS_prior = Ini_Read_Logical('use_PS_prior',.false.)
             ps_mean = Ini_read_Real(numcat('PS_mean',i),ps_mean_default)
             ps_stdev = Ini_read_Real(numcat('PS_stdev',i),ps_stdev_default)
          end if
          
          CLTemplate(i) = Ini_Read_String(numcat('cmb_dataset_CL',i), .false.) 
          if (CLTemplate(i)/='') then
           CLScale(i) = Ini_read_Real(numcat('cmb_dataset_CL_scale',i),1.0)
           call ReadCLTemplate(datasets(i), CLTemplate(i),CLScale(i))
           use_CL_prior = Ini_Read_Logical('use_CL_prior',.false.)
           cl_mean = Ini_read_Real(numcat('CL_mean',i),cl_mean_default)
           cl_stdev = Ini_read_Real(numcat('CL_stdev',i),cl_stdev_default)
          end if
        ...
        !! Add stop error if SNLS is used with non-independent data sets - spt.lps12
        ...
        !! then actually read the data if use_snls is set - spt.lps12
        IF (Use_SNLS) THEN
           Ini_fail_on_not_found = .FALSE.
...
# endif
        ...
        !! Then, right after the call Ini_Close - spt.lps12
        IF (Use_SNLS) THEN
        ...
        !! at the very end of program SolveCosmology  - spt.lps12
